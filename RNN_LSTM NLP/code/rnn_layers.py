...
def rnn_step_backward(dnext_h, cache):
    ...
    dWh = np.matmul(prev_h.T, np.multiply(dnext_h, (1 - next_h ** 2)))
...
def rnn_backward(dh, cache):
    ...
    for t in range(T, -1, -1):
        if t > 0:
            cache = x[t, :, :], h[t-1, :, :], Wx, Wh, b, h[t, :, :]
            dx_, dprev_h, dWx_, dWh_, db_ = rnn_step_backward(dh[t, :, :] + dprev_h, cache)
        else:
            cache = x[t, :, :], h0, Wx, Wh, b, h[t, :, :]
            dx_, dprev_h, dWx_, dWh_, db_ = rnn_step_backward(dh[t, :, :] + dprev_h, cache)
            dh0 = dprev_h
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    N, H = dprev_h.shape 
    dx = np.matmul(np.multiply(dnext_h, np.multiply(next_h, (1 - next_h))), Wx.T)
    dWx = np.matmul(np.multiply(np.multiply(next_h, (1 - next_h)), dnext_h), x.T)
    db = np.matmul(np.multiply(dnext_h, np.multiply(next_h, (1 - next_h))).T, np.ones(np.size(x)[0]))
    dWh = np.matmul(prev_h.T, np.multiply(dnext_h, np.multiply(next_h, (1 - next_h))))
    dprev_h = np.matmul(np.multiply(dnext_h, np.multiply(next_h, (1 - next_h))), Wh.T)
    dWf_t, dWx_i, dWc_t, dWo_t = dWx[:, H], dWx[:, H:2*H], dWx[:, 2*H:3*H], dWx[:, 3*H:4*H]
    dWf_t, dWx_i, dWc_t, dWo_t = dWx[:, H], dWx[:, H:2*H], dWx[:, 2*H:3*H], dWx[:, 3*H:4*H]
    dWf_t, dWx_i, dWc_t, dWo_t = dWx[:, H], dWx[:, H:2*H], dWx[:, 2*H:3*H], dWx[:, 3*H:4*H]
