...
class SVM(object):
  ...
  #       logit1 = np.matmul(X,self.params['W1']) + np.matmul(np.ones((np.shape(X)[0],1)), self.params['b1'])
  #       logit1_relu, relu_cache = relu_forward(logit1)
  #       logit2 = np.matmul(logit1_relu, np.transpose(self.params['W2'])) + self.params['b2']
  #       scores = logit2
  #   else:
  #       logit1 = np.matmul(X, self.params['W1']) + self.params['b1'] 
  #       scores = logit1    
  #   ############################################################################
  #   #                             END OF YOUR CODE                             #
  #   ############################################################################
  ...
  #   if 'W2' in self.params:
  #       grads['W2'] = np.matmul(np.transpose(grad1) , logit1)
  #       grads['b2'] = np.sum(grad1)
  #       # Pass through relu backward
  #       grads['W1'] = np.matmul(np.transpose(X), relu_backward(np.matmul(grad1, self.params['W2']), relu_cache))
  #       grads['b1'] = np.matmul(np.ones((1,len(y))), relu_backward(np.matmul(grad1, self.params['W2']), relu_cache)) 
  #   else:
  #       grads['W1'] = np.matmul(np.transpose(X) , grad1)
  #       grads['b1'] = np.sum(grad1)
  #   ############################################################################
  #   #                             END OF YOUR CODE                             #
  #   ############################################################################
